...
* Continue looking at topics related to machine learning
* We are going to cover __Resampling methods__
...
- Don't need to make assumptions about the population distribution (e.g., when we perform two samples t-test, we make the assumption that the populations from which the samples are drawn are normally distributed)
...
- Cross Validation (often used in machine learning)
...
- Sample (with replacement) Verizon group (ILEC) customers and non-Verizon group (CLEC) customers separately
...
- Guest Lecture: Amy Eyler on Qualitative Methods
